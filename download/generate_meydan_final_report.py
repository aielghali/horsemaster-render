...
nap_table_data = [
    ...
    [
        Paragraph(f"#{nap['draw']}", horse_style),
        Paragraph(nap['horse'], horse_style),
        Paragraph(f"السباق {nap['race']}", cell_style),
        Paragraph('80%', cell_style)
    ]
]
nap_table = Table(nap_table_data, colWidths=[2.5*cm, 5*cm, 2.5*cm, 2*cm])
...
nb_table_data = [
    [Paragraph('<b>الرقم</b>', header_style), 
     ...
     Paragraph('<b>الثقة</b>', header_style)],
    [
        Paragraph(f"#{nb['draw']}", horse_style),
        Paragraph(nb['horse'], horse_style),
        Paragraph(f"السباق {nb['race']}", cell_style),
        Paragraph('85%', cell_style)
    ]
]
nb_table = Table(nb_table_data, colWidths=[2.5*cm, 5*cm, 2.5*cm, 2*cm])
...
for race in predictions['races']:
    ...
    for pred in race['predictions']:
        ...
        pred_data.append([
            Paragraph(draw, horse_style),
            Paragraph(medal, cell_style),
            Paragraph(pred['horse'], horse_style),
            Paragraph(pred['confidence'], cell_style),
            Paragraph(pred['analysis'], cell_style_right)
        ])
    
    pred_table = Table(pred_data, colWidths=[1.5*cm, 1.3*cm, 4*cm, 1.5*cm, 7*cm])
