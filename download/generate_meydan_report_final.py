...
# Load predictions
with open('/home/z/my-project/download/meydan_final_predictions.json', 'r', encoding='utf-8') as f:
...
story.append(Spacer(1, 15))
story.append(Paragraph('20 فبراier 2026', ParagraphStyle(
    name='Date',
    fontName='Microsoft YaHei',
    fontSize=18,
    leading=24,
...
nap_table_data = [
    [Paragraph('<b>الرقم</b>', header_style), Paragraph('<b>الترشيح</b>', header_style), Paragraph('<b>السباق</b>', header_style), Paragraph('<b>التحليل</b>', header_style)],
    [
        Paragraph(f"#{nap_data['number']}", cell_style),
        Paragraph(nap_data['horse'], cell_style),
        Paragraph(f"السباق {nap_data['race']}", cell_style),
        Paragraph(nap_data['reason'], cell_style_right)
    ]
]
nap_table = Table(nap_table_data, colWidths=[1.5*cm, 3*cm, 2*cm, 10*cm])
...
next_table_data = [
    [Paragraph('<b>الرقم</b>', header_style), Paragraph('<b>الترشيح</b>', header_style), Paragraph('<b>السباق</b>', header_style), Paragraph('<b>التحليل</b>', header_style)],
    [
        Paragraph(f"#{next_best['number']}", cell_style),
...
next_table = Table(next_table_data, colWidths=[1.5*cm, 3*cm, 2*cm, 10*cm])
...
for race in predictions['races']:
    # Race header
    header_text = f"السباق {race['raceNumber']}: {race['raceName']}"
    story.append(Paragraph(f"<b>{header_text}</b>", section_style))
    
    # Race info
    info_text = f"⏰ {race['time']} | 💰 {race['prize']} | 📍 {race['surface']}"
    ...
    for pred in race['predictions']:
        pos_icon = "🥇" if pred['position'] == 1 else "🥈" if pred['position'] == 2 else "🥉"
        pred_data.append([
            Paragraph(f"#{pred['number']}", cell_style),
            Paragraph(pos_icon, cell_style),
            Paragraph(pred['horse'], cell_style),
            Paragraph(pred['jockey'], cell_style),
            Paragraph(pred['confidence'], cell_style)
        ])
    
    pred_table = Table(pred_data, colWidths=[1.2*cm, 1.2*cm, 4.5*cm, 3.5*cm, 1.5*cm])
    ...
    if race.get('valuePick'):
        story.append(Spacer(1, 6))
        vp = race['valuePick']
        story.append(Paragraph(f"💡 <b>خيار القيمة:</b> #{vp['number']} {vp['horse']}", body_style))
